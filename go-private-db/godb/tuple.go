...
func (td *TupleDesc) copy() *TupleDesc {
	var fieldsCopy = make([]FieldType, len(td.Fields))
...
func (t *Tuple) writeTo(b *bytes.Buffer) error {
	for _, field := range t.Fields {
		value, isString := field.(StringField)

		var err error
		if isString {
			padded := make([]byte, StringLength)
			copy(padded, []byte(value.Value))
			err = binary.Write(b, binary.LittleEndian, padded)
		} else {
			err = binary.Write(b, binary.LittleEndian, field)
		}
		if err != nil {
			return err
		}
...
func readTupleFrom(b *bytes.Buffer, desc *TupleDesc) (*Tuple, error) {
	...
	for _, field := range desc.Fields {
		if field.Ftype == StringType {
			...
			err := binary.Read(b, binary.LittleEndian, &strBytes)
			if err != nil {
				return nil, err
			}
			fields[index] = StringField{Value: string(bytes.TrimRight(strBytes[:], "\x00")[:])}
		} else {
			var val int64
			...
			fields[index] = IntField{Value: val}
		}
		index++
	}
	return &Tuple{Desc: *desc, Fields: fields}, nil //replace me
...
func (t1 *Tuple) equals(t2 *Tuple) bool {
	...
	for i := 0; i < len(t1.Fields); i++ {
		if !reflect.DeepEqual(t1.Fields[i], t2.Fields[i]) {
			return false
		}
	}
	return true
...
func (t *Tuple) compareField(t2 *Tuple, field Expr) (orderByState, error) {
	_tEval, err := field.EvalExpr(t)
	if err != nil {
		return OrderedEqual, err
	}
	_t2Eval, err := field.EvalExpr(t2)
	if err != nil {
		return OrderedEqual, err
	}

	if field.GetExprType().Ftype == StringType {
		tEval := _tEval.(StringField)
		t2Eval := _t2Eval.(StringField)
		if tEval.Value < t2Eval.Value {
			return OrderedLessThan, nil
		} else if tEval.Value > t2Eval.Value {
			return OrderedGreaterThan, nil
		} else {
			return OrderedEqual, nil
		}
	} else {
		tEval := _tEval.(IntField)
		t2Eval := _t2Eval.(IntField)
		if tEval.Value < t2Eval.Value {
			return OrderedLessThan, nil
		} else if tEval.Value > t2Eval.Value {
			return OrderedGreaterThan, nil
